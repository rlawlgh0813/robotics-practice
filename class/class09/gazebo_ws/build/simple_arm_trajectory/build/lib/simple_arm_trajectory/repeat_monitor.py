...
# 역방향 초기 세트: WP3 -> WP0 순서
WAYPOINTS = [
    [0.2, 0.3, -0.2, 0.0],   # WP 0 (최종 자세)
    [0.0, 0.3, -0.2, 0.0],   # WP 1
    [0.2, 0.1, 0.0, 0.0],    # WP 2
    [0.0, 0.0, 0.0, 0.0],    # WP 3 (Home)
]
...
class RepeatMonitor(Node):
    ...
    def _goal_cb(self, future, idx):
        ...
        res_fut.add_done_callback(lambda f: self._on_trajectory_done(f, idx))
    ...
    def _check_convergence(self, idx):
        ...
        if converged or timed_out:
            ...
            if timed_out and not converged:
                self.get_logger().warn(f'[경고] WP {idx} 수렴 실패')
